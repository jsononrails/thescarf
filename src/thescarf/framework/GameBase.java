...
import thescarf.framework.input.SafeKeyboardInput;
...
public class GameBase extends JFrame implements Runnable {
	...
	protected Matrix3x3f getViewportTransform() {
		...
		return GraphicsUtils.createViewport(appWorldWidth,
				appWorldHeight, 
				canvas.getWidth(),
				canvas.getHeight());
	...
	protected Matrix3x3f getReverseViewportTransform() {
		...
		return GraphicsUtils.createReversViewport(
				appWorldWidth, 
				appWorldHeight, 
				canvas.getWidth(), 
				canvas.getHeight());
	...
	protected Vector2f getWorldMousePosition() {
		...
		return screenToWorld.multiply(screenPos);
	...
	protected void render(Graphics g) {
		...
		textPos = GraphicsUtils.drawString(g, 20, 0, frameRate.getFrameRate());
	...
	protected static void launchApp(final GameBase app) {
